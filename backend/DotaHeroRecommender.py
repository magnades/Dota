...
class DotaHeroRecommender:
    ...
    def _get_players_strengths(self):
        return self.load_json("Data/players_heroes_strengths.json")
        ...
        if desired_role:
            for candidate in candidates:
                ...
                if desired_role in hero_roles:
                    scores[candidate] += 3  # Bonus de 1 punto por coincidir con el rol deseado
    ...
    def recommend_heroes_for_player(self, allies: list, enemies: list, position: int, player_name: str, num_recom: int) -> list:
        ...
        if desired_role:
            for candidate in candidates:
                ...
                hero_roles = hero.get("roles", [])
                if desired_role in hero_roles:
                    scores[candidate] += 3  # Bonus de 3 punto por coincidir con el rol deseado
